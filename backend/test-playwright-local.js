// Test de Playwright para bypass de Akamai INFONAVIT
...
async function testAkamaiBypass() {
  ...
  try {
    ...
    console.log('⏳ Paso 2: Esperando 5 segundos para generación de cookies Akamai...');
    await page.waitForTimeout(5000);

    // Verificar cookies generadas
    const cookies = await context.cookies();
    const akamaiCookies = cookies.filter(c => c.name.includes('bm_') || c.name.includes('_abck') || c.name.includes('ak_'));
    console.log(`✅ Cookies Akamai generadas: ${akamaiCookies.map(c => c.name).join(', ')}\n`);
   ...
   <soapenv:Body>
    ...
    if (response.status === 200) {
      console.log('🎉 ÉXITO: Akamai bypass funcional!\n');
